...
function WalletPage() {
  return <AppShell><div className="space-y-6">
    <PageHeader eyebrow="ALPHENTRA Economy" title="ALPHENTRA Wallet" description="Prototype balance, competition rewards, strategy payments, and future token utility — no real funds." actions={<Button asChild variant="outline"><Link to="/tournaments">View Competitions</Link></Button>} />
    <div className="grid gap-4 lg:grid-cols-3">
      <GlassCard className="lg:col-span-2 p-6 border-primary/20"><div className="flex items-start justify-between"><div><p className="text-sm text-muted-foreground">ALPHENTRA Balance</p><p className="mt-2 text-4xl font-semibold tracking-tight">1,250.00 <span className="text-lg text-muted-foreground">ALPH</span></p><p className="mt-2 text-xs text-amber-400">Prototype balance · no real funds</p></div><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10"><Coins className="h-6 w-6 text-primary" /></div></div><div className="mt-6 flex flex-wrap gap-2"><button disabled className="rounded-lg bg-primary px-4 py-2 text-sm font-medium opacity-60"><ArrowDownLeft className="mr-2 inline h-4 w-4" />Add ALPH (Soon)</button><button disabled className="rounded-lg border px-4 py-2 text-sm font-medium opacity-60"><ArrowUpRight className="mr-2 inline h-4 w-4" />Send (Soon)</button></div></GlassCard>
      <GlassCard className="p-6"><div className="flex items-center gap-3"><Trophy className="h-5 w-5 text-amber-400" /><h3 className="font-semibold">Competition Credits</h3></div><p className="mt-4 text-3xl font-semibold">1,150 <span className="text-sm text-muted-foreground">ALPH</span></p><p className="mt-2 text-sm text-muted-foreground">Prototype allocation reserved for ALPHENTRA Arena entry flows.</p></GlassCard>
    </div>
    <div className="grid gap-4 md:grid-cols-3">{walletCards.map((card) => { const Icon = card.icon; return <GlassCard key={card.title} className="p-5"><Icon className="h-5 w-5 text-primary" /><h3 className="mt-4 font-semibold">{card.title}</h3><p className="mt-2 text-sm text-muted-foreground">{card.text}</p></GlassCard>; })}</div>
    <GlassCard className="p-6"><h2 className="font-semibold">Recent activity</h2><div className="mt-4 divide-y divide-border/50">{transactions.map((transaction) => <div key={transaction.name + transaction.date} className="flex items-center justify-between py-4 text-sm"><div><p className="font-medium">{transaction.name}</p><p className="text-xs text-muted-foreground">{transaction.date}</p></div><div className="text-right"><p className={transaction.amount.startsWith("+") ? "text-success" : "text-foreground"}>{transaction.amount}</p><p className="text-xs text-muted-foreground">{transaction.status}</p></div></div>)}</div></GlassCard>
