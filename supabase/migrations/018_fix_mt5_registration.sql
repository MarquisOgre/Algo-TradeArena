-- Alphentra Database Migration 018
-- Fix ambiguous MT5 registration function output/column reference.
...
begin
  ...
  do update set
    ...
    status = 'pending',
    updated_at = now()
  returning public.broker_accounts.id into v_broker_id;
  ...
  on conflict on constraint mt5_accounts_broker_account_id_key
  do update set
    login_identifier = excluded.login_identifier,
    server_name = excluded.server_name,
    updated_at = now()
  returning public.mt5_accounts.id into v_mt5_id;
...
is 'Registers an authenticated users MT5 broker/account metadata without storing MT5 credentials; fixed column ambiguity.';
